module Fae
  module FormHelper
    ...
    def set_prompt(f, attribute, options)
      options[:prompt] = 'Zvolte' if is_association?(f, attribute) && f.object.class.reflect_on_association(attribute).macro == :belongs_to && options[:prompt].nil? && !options[:two_pane]
    end
